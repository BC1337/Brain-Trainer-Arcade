<script>
  ...
  const drawCanvas = () => {
    ...
    for (let box of boxes) {
      if (!box.clicked) {
        ...
        if (ctx.roundRect) {
            ctx.roundRect(box.x, box.y, box.size, box.size, 8);
        } else {
  ...
  const handleGameOver = async () => {
    ...
    if (isLoggedIn) {
      ...
      const token = localStorage.getItem('token');
      try {
        // MATCHED URL TO DASHBOARD: 'chimp-test'
...
<Layout>
  <div class="game-wrapper">
    ...
    <div class="canvas-container" style="width: {canvasWidth}px; height: {canvasHeight}px;">
      <canvas 
        bind:this={canvas} 
        on:mousedown|preventDefault={handleCanvasClick}
        class:dimmed={!gameStarted || showGameOverModal}
      ></canvas>
      ...
      {#if showGameOverModal}
        <div class="overlay">
          <div class="card">
            <h2 class="error-text">Test Failed</h2>
            ...
            <div class="sync-status">
              {#if isLoggedIn}
                <div class="status-indicator success">
                   <span>{isSaving ? 'Saving Score...' : 'Score Synced! ✔'}</span>
                </div>
              {:else}
                <div class="status-indicator guest">
                  <p>Guest Session</p>
                  <a href="/login" class="login-link">Login to track progress</a>
                </div>
              {/if}
            </div>
...
<style>
  ...
  .btn-primary { 
    background: var(--chimp-green); 
    color: white; 
    box-shadow: 0 4px 0 #0f763e;
  }
  ...
  .error-text { color: #ef4444; margin: 0 0 1rem; font-size: 1.8rem; font-weight: 900; }
